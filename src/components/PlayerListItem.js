	...
	return (
		<Grid className={classes.item} item xs={12}>
			<Card className={`${classes.card} ${active}`}>
				<CardContent
					onClick={() => storePatch(plItemPlay(playlistItemId))}
				...
				<IconButton
					...
					aria-label="Delete"
					onClick={() => deleteVid(isPlaying, playlistItemId)}
				>
					<IconWithSpin spin={isDeleting} color="secondary">
						<DeleteIcon />
					</IconWithSpin>
				</IconButton>
			</Card>
...
PlayerListItem.propTypes = {
	...
	playlistItemId: PropTypes.string.isRequired
};
