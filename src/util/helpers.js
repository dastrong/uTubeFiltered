// returns successful results or throws an error object
...
export function convertDateToString(date) {
  ...
  const splitDate = new Date(date).toDateString().slice(4).split("");
...
export function createTagDescription(channelIds, query) {
  const description = JSON.stringify({
    query,
    lastUpdate: Date.now(),
    channels: channelIds.reduce(
      (acc, cVal, i, arr) =>
